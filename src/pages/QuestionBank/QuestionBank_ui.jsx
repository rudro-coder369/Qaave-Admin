...
// Import your custom Tiptap Editor
...
// 🚀 SMART JSON RENDERER: Safely extracts text and math from Tiptap JSON
const RichTextPreview = ({ content }) => {
  if (!content) return null;

  let textContent = '';

  // Recursive function to dig into Tiptap JSON and extract plain text & math
  const extractText = (node) => {
    if (node.text) return node.text;
    if (node.type === 'mathNode') return `$$${node.attrs?.latex || ''}$$`; 
    if (node.type === 'image') return '[Image attachment] ';
    if (node.content) return node.content.map(extractText).join('');
    // Add line breaks for block elements
    if (['paragraph', 'heading', 'codeBlock'].includes(node.type)) return '\n';
    return '';
  };

  // Check if content is a JSON object or a stringified JSON
  if (typeof content === 'object' && content.type === 'doc') {
    ...
    try {
      ...
      if (parsed.type === 'doc') {
        textContent = parsed.content?.map(extractText).join('') || '';
      } else {
        textContent = content;
      }
    } catch (e) {
      textContent = content; // Fallback to raw string if it's not JSON
    }
  } else {
    textContent = String(content);
  }

  if (!textContent.trim()) return <span className="text-zinc-600 italic text-xs">No content</span>;

  // Split and render Math equations properly
  const parts = textContent.trim().split(/(\$\$[\s\S]*?\$\$|\$[\s\S]*?\$)/g);
    ...
    <div className="flex flex-col h-auto min-h-[calc(100vh-100px)] lg:h-[calc(100vh-100px)] lg:min-h-0 text-zinc-300 font-sans overflow-y-auto lg:overflow-hidden p-3 lg:p-4 bg-transparent">
      ...
      {!selectedChap ? (
        ...
        <div className="flex-1 flex flex-col lg:grid lg:grid-cols-12 min-h-0 pb-2">
          ...
          <div className="lg:col-span-6 xl:col-span-7 flex flex-col h-[60vh] lg:h-full bg-transparent overflow-hidden shrink-0 lg:shrink lg:border-r border-zinc-800/60 lg:pr-6">
                ...
                filteredQuestions.map((q, idx) => (
                  <div key={q.id} className={`py-5 border-b border-zinc-800/50 transition-all duration-200 group relative ${editingId === q.id ? 'bg-zinc-900/40 px-4 -mx-4 border-emerald-500/30' : 'hover:bg-zinc-900/20 px-4 -mx-4'}`}>
                    ...
                    <div className="flex gap-3 mb-4">
                      ...
                      <div className="min-w-0 flex-1"><RichTextPreview content={q.question_text} /></div>
                    </div>
                          ...
                          return (
                            <div key={i} className="flex gap-3">
                              ...
                              <div className="flex flex-col gap-1.5 min-w-0">
                                {text && <RichTextPreview content={text} />}
                      ...
                      <div className="ml-6 mt-2">
                          ...
                          {q.mcq_options?.map((opt, i) => (
                            <div key={opt.id} className={`p-3 rounded border flex items-start gap-3 transition-colors ${opt.is_correct ? 'bg-emerald-500/5 border-emerald-500/30 text-emerald-100' : 'bg-zinc-900/30 border-zinc-800/50 text-zinc-400'}`}>
                              ...
                              <div className="flex flex-col gap-1.5 w-full mt-0.5 min-w-0">
                                {opt.option_text && <RichTextPreview content={opt.option_text} />}
                        ...
                        {q.explanation && (
                          <div className="mt-4 p-3.5 bg-zinc-900/30 rounded border border-zinc-800/50 flex gap-3 text-zinc-300">
                            ...
                            <div className="min-w-0"><RichTextPreview content={q.explanation} /></div>
                          </div>
                        )}
                      </div>
                    )}
                        ...
                        {q.cq_parts?.map(p => (
                          <div className="flex flex-col gap-2 p-4 bg-zinc-900/30 rounded border border-zinc-800/50" key={p.id}>
                            <div className="flex gap-2">
                              ...
                              <div className="min-w-0"><RichTextPreview content={p.question_text} /></div>
                            </div>
                            {p.answer_text && (
                              <div className="pl-6 pt-2 mt-2 border-t border-zinc-800/50 flex gap-2">
                                ...
                                <div className="min-w-0"><RichTextPreview content={p.answer_text} /></div>
                              </div>
                            )}
                          </div>
                        ))}
                    ...
                    {['sq', 'written'].includes(q.q_type) && (
                      <div className="mt-4 ml-6 p-4 bg-zinc-900/30 rounded border border-zinc-800/50 flex gap-3">
                        ...
                        <div className="min-w-0"><RichTextPreview content={q.solution} /></div>
                      </div>
                    )}
                  </div>
                ))
          ...
          {/* 📝 FORM EDITOR SECTION */}
          <div className="lg:col-span-6 xl:col-span-5 flex flex-col h-[75vh] lg:h-full overflow-hidden bg-transparent shrink-0 lg:shrink lg:pl-6 pt-6 lg:pt-0">
            <div className="pb-3 mb-2 border-b border-zinc-800/60 shrink-0">
              <div className="flex bg-[#0a0a0a] p-1 rounded border border-zinc-800/50">
                {[{ id: 'mcq', label: 'MCQ' }, { id: 'sq1', label: 'Short Q (1 Mark)' }, { id: 'sq2', label: 'Written Q (2 Marks)' }, { id: 'cq', label: 'Creative' }].map(tab => (
                  <button key={tab.id} type="button" onClick={() => setQType(tab.id)} className={`flex-1 py-1.5 text-[9px] md:text-[10px] font-bold uppercase tracking-widest transition-all rounded ${qType === tab.id ? 'bg-zinc-800 text-zinc-100 border border-zinc-700/50' : 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-900/50'}`}>{tab.label}</button>
                ))}
              </div>
            </div>
            ...
            <form onSubmit={handleAddOrUpdateQuestion} className="flex-1 overflow-y-auto flex flex-col gap-6 custom-scrollbar pr-2 pb-10">
              ...
              <div className="flex flex-col gap-2">
                ...
                <div className="border border-zinc-800 rounded bg-[#0a0a0a] overflow-hidden focus-within:border-emerald-500/50 transition-colors">
                    <LearningEditor initialContent={newQ.text} onUpdate={(json) => setNewQ({...newQ, text: json})} />
                </div>
                      ...
                      {mcqStatements.map((stmt, idx) => (
                        <div key={idx} className="flex gap-3 items-start">
                          ...
                          <div className="flex flex-col gap-2 flex-1 min-w-0">
                            <div className="border border-zinc-800 rounded bg-[#0a0a0a] overflow-hidden focus-within:border-emerald-500/50 transition-colors">
                                <LearningEditor initialContent={stmt.text} onUpdate={(json) => setMcqStatements(mcqStatements.map((s, i) => i === idx ? {...s, text: json} : s))} />
                            </div>
              ...
              <div className="flex flex-col gap-4 border-t border-zinc-800/80 pt-5">
                  ...
                  <div className="space-y-4">
                    ...
                    {options.map((opt, idx) => (
                      <div key={idx} className="flex items-start gap-3">
                        ...
                        <div className="flex flex-col gap-2 flex-1 min-w-0">
                          <div className={`rounded overflow-hidden transition-colors border ${opt.isCorrect ? 'bg-emerald-500/5 border-emerald-500/50' : 'bg-[#0a0a0a] border-zinc-800 focus-within:border-zinc-600'}`}>
                             <LearningEditor initialContent={opt.text} onUpdate={(json) => setOptions(options.map((o, i) => i === idx ? {...o, text: json} : o))} />
                          </div>
                    ...
                    <div className="pt-3">
                       ...
                       <div className="border border-zinc-800 rounded bg-[#0a0a0a] overflow-hidden focus-within:border-emerald-500/50 transition-colors">
                            <LearningEditor initialContent={newQ.explanation} onUpdate={(json) => setNewQ({...newQ, explanation: json})} />
                       </div>
                    </div>
                  </div>
                )}
                ...
                {['sq1', 'sq2'].includes(qType) && (
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="border border-zinc-800 rounded bg-[#0a0a0a] overflow-hidden focus-within:border-emerald-500/50 transition-colors">
                        <LearningEditor initialContent={newQ.solution} onUpdate={(json) => setNewQ({...newQ, solution: json})} />
                    </div>
                  </div>
                )}
                    ...
                    {cqParts.map((part, idx) => (
                      <div key={idx} className="flex flex-col gap-3">
                        <div className="flex gap-3 items-center">
                          ...
                          <div className="flex-1 border border-zinc-800 rounded bg-[#0a0a0a] overflow-hidden focus-within:border-emerald-500/50 transition-colors">
                              <LearningEditor initialContent={part.qText} onUpdate={(json) => setCqParts(cqParts.map((p, i) => i === idx ? {...p, qText: json} : p))} />
                          </div>
                        </div>
                        <div className="flex gap-3 items-start">
                          <span className="text-[9px] font-bold text-zinc-600 w-5 pt-3 uppercase tracking-widest shrink-0">Ans:</span>
                          <div className="flex-1 border border-zinc-800 rounded bg-[#0a0a0a] overflow-hidden focus-within:border-emerald-500/50 transition-colors">
                              <LearningEditor initialContent={part.aText} onUpdate={(json) => setCqParts(cqParts.map((p, i) => i === idx ? {...p, aText: json} : p))} />
                          </div>
                        </div>
                      </div>
                    ))}
...
// ... DatabaseIcon & FolderOpenIcon components remain same
